...
export async function POST(request: Request) {
  try {
    const { id, status, reason, userId, amount } = await request.json() as WithdrawalStatusInput;

    // input validation
    if (!id || !status || !userId || amount <= 0) {
    ...
    if (status === 'completed') {
      ...
      const walletAddress = deposit.wallet_address;
